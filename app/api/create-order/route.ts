...
export async function POST(req: Request) {
  ...
  // This project requires login before checkout — the server-verified session
  // cookie is the only source of truth for order ownership. Never create an
  // order without it (no anonymous/ownerless fallback).
  ...
  try {
    ...
    let rpOrder: { id: string; amount: string | number; currency: string } | null = null;
    if (!isCod) {
    ...
    // COD has no Razorpay capture event to audit — skip the payment_transactions row.
    if (rpOrder) {
      try {
    ...
    return jsonOk({
      ...
      amount: rpOrder?.amount ?? Math.round(total * 100),
      currency: rpOrder?.currency ?? "INR",
      key: isCod ? null : process.env.NEXT_PUBLIC_RAZORPAY_KEY_ID,
    });
  } catch (err) {
