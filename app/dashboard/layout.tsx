...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  const { isAuthenticated, userPhone, userName, logout, updateProfile } = useAuthStore();
  ...
  // Keep the store's cached name/phone in sync with the DB so every dashboard
  // page (sidebar, addresses, orders) reflects the latest saved profile
  // without each one re-fetching /api/user itself.
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // best-effort — keep whatever is already cached in the store
      }
    })();
    return () => {
      cancelled = true;
  ...
  return (
    <div className="pt-[30px] md:pt-[30px] lg:pt-[30px] min-h-screen font-outfit">
      <div className="mx-[15px] lg:mx-[50px] flex flex-col lg:flex-row">
