...
export default function DashboardOverview() {
  const { userName, login, updateProfile } = useAuthStore();
  ...
  const otpFlow = useOtpFlow({
    verifyEndpoint: "/api/user/phone",
    verifyMethod: "PATCH",
    onVerified: async (verifiedPhone, data) => {
      const savedName: string = data?.user?.name ?? "";
      if (pendingName && pendingName !== savedName) {
        try {
          ...
          toast.error("Phone updated, but failed to save name");
        }
      }
      setOriginalPhone(verifiedPhone);
      setOriginalName(pendingName || savedName);
      login(verifiedPhone, pendingName || savedName);
      updateProfile({ userName: pendingName || savedName });
  ...
  const handleSave = async () => {
    ...
    // If phone changed, an OTP must verify ownership of the new number before it's saved.
    if (phoneChanged) {
      setPendingName(name.trim());
      setShowOtpModal(true);
      otpFlow.reset();
      otpFlow.setPhoneDigit(cleanPhone);
      return;
    }
  ...
  return (
    <div className="bg-[#E6F7F4] w-full px-[15px] py-[20px] md:px-[20px] md:py-[30px] lg:p-[60px] font-outfit shadow-sm h-full min-h-[400px]">
      <h1 className="txt-h1 leading-none lg:leading-[22px] tracking-[0.03em] font-medium text-[#121212]  mb-[35px] lg:mb-[45px]">
      ...
      {loading ? (
        ...
        <div className="max-w-[1062px] flex flex-col gap-6">
          ...
          <div className="flex items-center gap-[15px] mt-[24px] lg:mt-[20px]">
            <button
              onClick={handleSave}
      ...
      {showOtpModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          <div
            ...
            onClick={() => otpFlow.loading === "idle" && closeOtpModal()}
          />
          <div className="relative z-10 w-full max-w-[480px] bg-white pt-[35px] pb-[25px] px-[30px] md:px-[45px] shadow-2xl">
            ...
            <h2 className="txt-h2 font-bold text-[#121212] mb-[20px] tracking-wide">
