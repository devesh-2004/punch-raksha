...
export function CartDrawer() {
  ...
  // Shared "log in, then resume checkout" flow — used both for a first-time
  // checkout click and for re-prompting after the server session turns out
  // to be stale/expired (see handleCheckoutClick and payNow below).
  ...
  const handleCheckoutClick = async () => {
    // The persisted `isAuthenticated` flag (localStorage) can go stale — e.g. the
    // 7-day session cookie expires naturally with nothing to clear this client
    // flag. Always confirm there is still a live server session before letting
    // checkout proceed; the cookie is the actual source of truth.
  ...
  const payNow = async () => {
    ...
    if (method === "cod") {
      ...
      try {
        ...
        localStorage.setItem("lastOrder", JSON.stringify({ orderId: data.orderId }));
        clearCart();
        closeDrawer();
        setTimeout(() => {
          setFlowStep("cart");
    ...
    const options: any = {
      ...
      handler: async function (response: any) {
        try {
          ...
          localStorage.setItem("lastOrder", JSON.stringify({ orderId: orderToUse.orderId }));
          clearCart();
          closeDrawer();
          setTimeout(() => {
            setFlowStep("cart");
  ...
  const handleSaveAddress = (val: NewAddress) => {
    ...
    // First address ever added — save the name to the account profile too
    if (!editingAddressId && isAuthenticated && !userName && val.fullName.trim()) {
      fetch("/api/user", {
        method: "PATCH",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ name: val.fullName.trim() }),
      })
        .then((res) => res.json())
        .then((data) => {
          if (!data?.error) updateProfile({ userName: val.fullName.trim() });
        })
        .catch(() => {});
    }

    // Persist new addresses to the account's address book so they also show on the dashboard
