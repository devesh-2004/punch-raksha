...
import {
  ...
  PLATFORM_ADDITIONAL_DISCOUNT_PERCENT,
  PLATFORM_ADDITIONAL_DISCOUNT_MAX,
} from "@/lib/utils/discountCalc";
...
export function PaymentOffers({
  ...
  const upiTotalPct = upiPct + PLATFORM_ADDITIONAL_DISCOUNT_PERCENT;
  const upiTotalMax = upiMax + PLATFORM_ADDITIONAL_DISCOUNT_MAX;
  const cardTotalPct = cardPct + PLATFORM_ADDITIONAL_DISCOUNT_PERCENT;
  const cardTotalMax = cardMax + PLATFORM_ADDITIONAL_DISCOUNT_MAX;
  ...
  const offers = [
    {
      ...
      label: "Pay via UPI",
      discount: upiDiscount > 0 ? `${upiTotalPct}% off upto ₹${upiTotalMax}` : "No extra discount",
    ...
    },
    {
      ...
      original: formatPrice(subtotal),
      label: "Pay via Card",
      discount: cardDiscount > 0 ? `${cardTotalPct}% off upto ₹${cardTotalMax}` : "No extra discount",
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="flex items-start gap-3 rounded-[5px] custom-dashed-border bg-[#f8fcf9] p-3 md:p-[10px]">
          ...
          <div className="font-outfit txt-p-lg font-normal text-[#121212]">
            Pay online :{" "}
            <span className="font-semibold">{Math.max(upiPct, cardPct) + PLATFORM_ADDITIONAL_DISCOUNT_PERCENT}% more discount</span> & Faster
            delivery
          </div>
        </div>
      </div>
          ...
          >
            ...
            >
              ...
              <div className="flex flex-col items-end justify-between md:flex-row md:items-end gap-2 md:gap-4">
                <div
                  className={`payment-offer-badge px-[10px] py-[5px] md:p-[10px] font-outfit text-[14px] max-md:tracking-[0.03em] md:text-[16px] mb-[5px] md:mb-[0px] font-medium ${
