...
import { Sep, TB, Modal } from "./editor/ToolbarUI";
import {
  CtaBannerNode,
  CtaBannerFields,
  CTA_BANNER_DEFAULTS,
  type CtaBannerAttrs,
} from "./editor/blocks/ctaBannerBlock";
import { FaqAccordionNode, FaqItemNode, FaqQuestionNode, FaqAnswerNode } from "./editor/blocks/faqAccordionBlock";
import { CalloutNode, CalloutFields, CALLOUT_DEFAULTS, type CalloutAttrs } from "./editor/blocks/calloutBlock";
import { TipBoxNode, TipBoxFields, TIP_BOX_DEFAULTS, type TipBoxAttrs } from "./editor/blocks/tipBoxBlock";
...
const EDITOR_STYLES = `
  ...
  /* Editing-only cell outline so authors can see column/row boundaries; the
     published page (no .tiptap-pr-editor wrapper) stays borderless per design. */
  .tiptap-pr-editor th, .tiptap-pr-editor td { border: 1px dashed #d1d5db; }
...
export function RichTextEditor({ value, onChange }: Props) {
  const [dialog, setDialog] = useState<null | "link" | "image" | "youtube" | "cta-banner" | "callout" | "tip-box">(null);
  const [linkUrl, setLinkUrl] = useState("");
  const [imageUrl, setImageUrl] = useState("");
  const [ytUrl, setYtUrl] = useState("");
  const [ctaDraft, setCtaDraft] = useState<CtaBannerAttrs>(CTA_BANNER_DEFAULTS);
  const [calloutDraft, setCalloutDraft] = useState<CalloutAttrs>(CALLOUT_DEFAULTS);
  const [tipBoxDraft, setTipBoxDraft] = useState<TipBoxAttrs>(TIP_BOX_DEFAULTS);
  ...
  const openCtaDialog = () => {
    setCtaDraft(CTA_BANNER_DEFAULTS);
    setDialog("cta-banner");
  };

  const insertCtaBanner = () => {
    if (!editor) return;
    editor.chain().focus().setCtaBanner(ctaDraft).run();
    setDialog(null);
  };
  ...
  const openCalloutDialog = () => {
    setCalloutDraft(CALLOUT_DEFAULTS);
    setDialog("callout");
  };

  const insertCallout = () => {
    if (!editor) return;
    editor.chain().focus().setCallout(calloutDraft).run();
    setDialog(null);
  };

  // ── Tip Box ───────────────────────────────────────────────────────────────

  const openTipBoxDialog = () => {
    setTipBoxDraft(TIP_BOX_DEFAULTS);
    setDialog("tip-box");
  };

  const insertTipBox = () => {
    if (!editor) return;
    editor.chain().focus().setTipBox(tipBoxDraft).run();
    setDialog(null);
  };

  // ── FAQ Accordion ─────────────────────────────────────────────────────────

  const insertFaqAccordion = () => {
    if (!editor) return;
    editor.chain().focus().insertFaqAccordion().run();
  };
  ...
  return (
    <div className="border border-gray-200 rounded-lg focus-within:border-[#045830] transition-colors">
      ...
      <div className="bg-gray-50 border-b border-gray-200 px-2 py-1.5 flex flex-wrap items-center gap-0.5">
        ...
        <TB onClick={() => editor.chain().focus().toggleBulletList().run()} active={editor.isActive("bulletList")} title="Arrow Bullet List (▶)">
          <Play size={13} />
        </TB>
        ...
        <TB onClick={openCtaDialog} title="Insert CTA Banner">
          <Megaphone size={13} />
        </TB>
        <TB onClick={insertFaqAccordion} title="Insert FAQ Accordion">
          <HelpCircle size={13} />
        </TB>
        <TB onClick={openCalloutDialog} title="Insert Callout (e.g. Read more)">
          <Info size={13} />
        </TB>
        <TB onClick={openTipBoxDialog} title="Insert Tip Box">
          <Lightbulb size={13} />
        </TB>
      ...
      {/* ── Table toolbar (only while the cursor is inside a table) ── */}
      {editor.isActive("table") && (
        <div className="bg-[#EDF9F5] border-b border-gray-200 px-2 py-1.5 flex flex-wrap items-center gap-0.5">
          <TB onClick={() => editor.chain().focus().addColumnBefore().run()} title="Add Column Before">
            <ArrowLeftToLine size={13} />
          </TB>
          <TB onClick={() => editor.chain().focus().addColumnAfter().run()} title="Add Column After">
          ...
          <TB onClick={() => editor.chain().focus().addRowBefore().run()} title="Add Row Above">
            <ArrowUpToLine size={13} />
          </TB>
          <TB onClick={() => editor.chain().focus().addRowAfter().run()} title="Add Row Below">
      ...
      {/* ── Content area ── */}
      {/* Bounded height + its own scrollbar so the toolbar above never depends
          on the surrounding page's scroll behavior (the admin shell's flex
          layout can grow past the viewport for long articles, dragging the
          sticky toolbar out of view with it). */}
      ...
      {/* ── CTA Banner dialog ── */}
      {dialog === "cta-banner" && (
        <Modal title="Insert CTA Banner" onClose={() => setDialog(null)}>
          <CtaBannerFields value={ctaDraft} onChange={setCtaDraft} />
          <button
            type="button"
      ...
      {/* ── Callout dialog ── */}
      {dialog === "callout" && (
        <Modal title="Insert Callout" onClose={() => setDialog(null)}>
          <CalloutFields value={calloutDraft} onChange={setCalloutDraft} />
          <button
            type="button"
      ...
      {/* ── Tip Box dialog ── */}
      {dialog === "tip-box" && (
        <Modal title="Insert Tip Box" onClose={() => setDialog(null)}>
          <TipBoxFields value={tipBoxDraft} onChange={setTipBoxDraft} />
          <button
            type="button"
