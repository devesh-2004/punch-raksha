...
import { uploadResizedImageToR2, uploadFileToR2 } from "@/lib/utils/uploadImage";

export type TabItem = {
  text: string;   // supports HTML: <b>, <i>, <span style="color:red"> etc.
  image: string;  // URL — if present shows photo circle, else shows green checkmark
...
export function GenericTabEditor({ data, onChange, importantNotes, onImportantNotesChange }: Props) {
  ...
  const handleImageUpload = async (idx: number, e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.type === "image/svg+xml" || file.name.toLowerCase().endsWith(".svg")) {
      // Upload as-is (no canvas resize — that would rasterize the vector). InlineImage
      // fetches the SVG from its R2 URL and inlines the markup so it stays CSS-styleable.
      try {
        updateItem(idx, "image", await uploadFileToR2(file));
      } catch (err) {
    ...
    // Raster images — resize + upload to R2, preserving the source format
    try {
      updateItem(idx, "image", await uploadResizedImageToR2(file, 400));
    } catch (err) {
      toast.error((err instanceof Error ? err.message : "Image upload failed"));
    }
  };
